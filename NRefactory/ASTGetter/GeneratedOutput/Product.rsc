...
,
memberDeclaration(fieldDeclaration("IsOnSale",[],[],[modifiersPublic(),],
[variableInitializer("IsOnSale",primitiveExpression(false))],primitiveType("bool")))
,
memberDeclaration(fieldDeclaration("_price",[],[],[modifiersPrivate(),],
[variableInitializer("_price",emptyExpression())],primitiveType("double")))
,
memberDeclaration(propertyDeclaration("Price",[],accessor([],blockStatementPlaceholder(
[returnStatement(conditionalExpression(identifierExpression("IsOnSale",[]),identifierExpression("_price",[]),binaryOperatorExpression(identifierExpression("_price",[]),multiply(),primitiveExpression(0.8),),))]),[],[modifiersNone(),],)
